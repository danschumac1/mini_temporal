...
actual = []
with open('aqatest') as iFile:
    for row in iFile:
        actual.append([' '.join(re.findall(r"\w+", row.strip())).lower()])
print(actual)
...
#model_generations = pd.read_json(f'~/mini_temporal/generations/trial.jsonl',  lines=True)
model_generations = pd.read_json(f'test_out.out',  lines=True)
#preds = []
#with open('test_out.out') as iFile:
#    for row in iFile:
#        preds.append(row.strip())

# print('\n\n', model_generations.columns,'\n\n')

preds = [' '.join(re.findall(r"\w+", pred)).lower() for pred in model_generations['PREDICTION']]
