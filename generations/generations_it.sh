...
base_models=("gemma-1.1-7b-it" "gemma-1.1-2b-it") # Add other models if needed
  ...
  for base_model in "${base_models[@]}"; do
    ...
    for trained_context in "${trained_contexts[@]}"; do
      ...
      for eval_context in "${eval_contexts[@]}"; do
        ...
        CUDA_VISIBLE_DEVICES=1 python /home/dan/mini_temporal/generations/generations_it.py \
          --dataset ${dataset} \
...
#### FOR DEBUGGING -------------------------------------------------
# nohup sh -c "CUDA_VISIBLE_DEVICES=1 python generations_it.py \
# --dataset TQE \
# --context no_context \
# --batch_size 32 \
# --model_path /home/dan/mini_temporal/training/models/TQE/gemma-1_1-7b-it/no_context" > "trial.jsonl" 2>&1 
### ----------------------------------------------------------------