        ...
        if context:
            for question, context, answer in zip(df['question'], df[context], df['answer']):
                if pd.isna(question) or pd.isna(context) or pd.isna(answer):
                    print(f"Skipping row with missing data: question={question}, context={context}, answer={answer}")
                    tokenized_instructions.append(None)
                else:
                    tokenized_instructions.append(f"{question} Here is the context: {context} The answer is: {answer}")
        else:
            for question, answer in zip(df['question'], df['answer']):
                if pd.isna(question) or pd.isna(answer):
                    print(f"Skipping row with missing data: question={question}, answer={answer}")
                    tokenized_instructions.append(None)
                else:
                    tokenized_instructions.append(f"{question} The answer is: {answer}")

    
    else: # THE MODEL IS AN INSTRUCTION TUNED MODEL
    # set up tokenizer
...
def main(): 
    ...
    test['rel_prompt'] = format_instruction_test(test, model_path = args.model_path, context = 'relevant_context')
    test['rand_prompt'] = format_instruction_test(test, model_path = args.model_path, context = 'random_context')
    test['wd_prompt'] = format_instruction_test(test, model_path = args.model_path, context = 'wrong_date_context')

    # TOKENIZATION
    tokenizer = AutoTokenizer.from_pretrained(f"./training/models/{args.model_path}", add_bos_token=False, add_eos_token=False)
    ...
    for i, batch in enumerate(loader):
        ...
        for j, item in enumerate(decoded_responses):
            ...
            if 'it' not in args.model_path: # BASE MODEL
                question = item.split('The answer is: ')[0].strip()
                answer = 'The answer is: ' +  item.split('The answer is: ')[1]

            else: # IT IS AN INSTRUCTION TUNED MODEL
                question = item.split('user\n')[1].strip()
                answer = item.split('user\n')[2].strip()
