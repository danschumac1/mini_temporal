...
def main(): 
    ...
    test['no_prompt'] = generate_no_context_prompt(test )
    test['rel_prompt'] = generate_context_prompt(test, 'relevant_context')
    test['rand_prompt'] = generate_context_prompt(test, 'random_context')
    test['wd_prompt'] = generate_context_prompt(test, 'wrong_date_context')
    ...
    for i, batch in enumerate(loader):
        ...
        for j, item in enumerate(decoded_responses):
            ...
            question = item.split('<|im_end|>')[0][18:]
            answer = item.split('<|im_end|>')[1]
