...
def parse_args():
    ...
    parser.add_argument('--base_model', type = str, required=True, choices= ['gemma-2b','gemma-1.1-2b-it','gemma-1.1-7b','gemma-7b-it'])
...
#                     output = fi_tokenizer.batch_decode(encodeds, special_tokens=True)
#                     tokenized_instructions.append(output[0] + '<eos>')
#     print(f"Formatted {len(tokenized_instructions)} instructions")
    ...
    if '-it' not in model_path:
        ...
        if context:
            for question, context, answer in zip(df['question'], df[context], df['answer']):
                ...
                #     tokenized_instructions.append(None)
                # else:
                tokenized_instructions.append(f"<bos>{question} Here is the context: {context} The answer is: {answer} <eos>")
        ...
        # BASE WITHOUT CONTEXT
            ...
            for question, answer in zip(df['question'], df['answer']):
                ...
                #     tokenized_instructions.append(None)
                # else:
                tokenized_instructions.append(f"<bos>{question} The answer is: {answer} <eos>")
...
def main():
    ...
    new_model = f"./training/models/{args.base_model.replace('.','_')}/{args.model_context}"
