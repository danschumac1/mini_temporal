...
for base_model in "${base_models[@]}"; do
  for context in "${contexts[@]}"; do
    ...
    nohup bash -c "CUDA_VISIBLE_DEVICES=1 python fine_tune_gemma_it.py --dataset AQA --epochs 4 --model_context $context --base_model $base_model --batch_size 16" >> $log 2>&1
  done
done
